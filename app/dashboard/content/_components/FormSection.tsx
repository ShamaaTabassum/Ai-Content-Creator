...
const FormSection = ({selectedTemplate,userFormInput,loading}:SELECTEDTEMPPROPS) => {

  const [formData,setFormData]=useState<any>();

  const iconSrc = selectedTemplate?.icon || '/blog-title.png';
  const name = selectedTemplate?.name || 'Default Name';
  const desc = selectedTemplate?.desc || 'Default Description';
...
setFormData({...formData,[name]:value})
  ...
  return (
    <div className='p-5 shadow-md border rounded-lg bg-white'>
      ...
      <p className='text-grey-500 text-sm'>{desc}</p>
